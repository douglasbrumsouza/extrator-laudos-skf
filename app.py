...
/* Espaçamento do container principal (Aumentado no topo para NUNCA cortar as labels) */
.block-container {
...
/* Gap vertical padrão nativo para não espremer os blocos */
...
.act-bar {
    ...
    height: 42px; /* Altura exata dos botões nativos */
}
.act-title {
    font-family: 'Barlow Condensed',sans-serif; font-size: 14px;
...
/* ── KPI CARDS ── */
.kpi { border-radius: 10px; padding: 14px 10px; text-align: center;
...
/* ── CAIXAS DOS GRÁFICOS (Sem colisão) ── */
...
.sc-top {
    ...
    border-radius:10px 10px 0 0; padding:12px 14px 20px 14px; /* Padding extra em baixo para o gráfico subir seguro */
}
div[data-testid="stPlotlyChart"] {
    background: #112035; border: 1px solid #1A3A5C; border-top: none;
    border-radius: 0 0 10px 10px; padding: 0 10px 10px 10px;
    margin-top: -20px !important; /* Desliza suavemente sobre o padding do título */
...
/* ── SELECTBOX LABELS (Limpo e nativo) ── */
...
def render_dashboard(lista):
    ...
    # ── FILTROS (Malha Nativa 2:1:1 para Alinhamento Perfeito) ─────────────────
    ...
    # ── KPIs (Colunas aninhadas matemáticas para respeitar a mesma malha) ──────
    kc1, kc2, kc3 = st.columns([2, 1, 1])
    
    with kc1:
        # Colunas aninhadas nativas do Streamlit resolvem perfeitamente a matemática de divisão!
        kc1_sub1, kc1_sub2 = st.columns(2)
        with kc1_sub1:
            st.markdown(f"""
    ...
    # ── GRÁFICOS (Malha Nativa 2:1:1 para fechar o alinhamento com a régua) ────
