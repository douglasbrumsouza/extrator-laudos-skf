...
/* ── CAIXAS DOS GRÁFICOS (Truque de Fusão Seguro sem Margens Negativas Invasivas) ── */
...
.sc-top {
    ...
    padding:12px 14px 28px 14px; /* O preenchimento estica o fundo azul escuro para baixo */
    margin-bottom: -28px; /* Anula o preenchimento apenas visualmente, para o gráfico escorregar por cima */
}
div[data-testid="stPlotlyChart"] {
    background: #112035; border: 1px solid #1A3A5C; border-top: none;
    border-radius: 0 0 10px 10px; padding: 0 10px 10px 10px;
    position: relative; z-index: 10; /* Mantém o gráfico na frente do fundo esticado */
}
...
def render_dashboard(lista):
    ...
    # ── KPIs (Colunas com Flexbox para respeitar a mesma malha vertical) ───────
    kc1, kc2, kc3 = st.columns([2, 1, 1])
    
    with kc1:
        # Flexbox limpo garante a divisão 50/50 SEM inserir divs invisíveis e bugadas do Streamlit
        st.markdown(f"""
    ...
    with gc1:
        ...
        if not dff.empty:
            ...
            # Range aumentado para 115 garante que textos de "100%" NUNCA cortem na margem da caixa
